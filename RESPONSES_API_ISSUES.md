...
> 注：工具定义采用**扁平格式**这一点 SenseAudio 是符合官方规范的（与 Assistants API 一致）。问题出在与**自家** `/chat/completions` 端点（嵌套格式）不一致，导致生态内工具定义无法直接复用。

### 2.2 SenseAudio 的支持范围

- 仅 `supports_responses=true` 的模型可用（动态探测自 `/v1/models`，当前：`qwen3.7-max`、`qwen3.8-max`、`deepseek-v4-flash-0731`）；
- 插件侧开关：`senseaudio.enableResponsesApi`（默认关闭），`senseaudio.apiMode = "responses"` 可强制；
...
SenseAudio 端点会拒绝这些块，**只接受 `input_text` / `output_text` / `input_image` 三种**。
...
> "SenseAudio 的 Responses 端点仍在演进（不同模型流式事件类型不一致、工具调用不稳定、多轮工具回填非常规）"
...
| 能力 | OpenAI 官方 `/v1/responses` | SenseAudio `/v1/responses` | 对插件的影响 |
|------|------------------------------|------------------------------|-------------|
