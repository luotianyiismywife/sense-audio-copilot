...
import type { ModelPreset, SenseAudioModelItem } from "./types";
...
export class SenseAudioChatModelProvider implements LanguageModelChatProvider {
    ...
    async provideLanguageModelChatResponse(
        ...
        try {
            ...
            // senseaudio.apiMode = "auto" follows each model's default; "openai"/"anthropic"/"responses" force the protocol.
            ...
            const enableThirdPartyIndicator = config.get<boolean>("senseaudio.enableThirdPartyTokenIndicator", true);
            ...
            while (true) {
                ...
                // 2. Proactive balance pre-check — DISABLED.
                //    SenseAudio 平台无 cookie 余额接口（/api/usage-summary 为旧平台
                //    TokenRhythm 专属），余额不足由 API 返回 402 触发被动轮换兜底。
                const balanceCheckDisabled = false; // set true to re-enable proactive balance pre-check
                if (balanceCheckDisabled && getBalanceCheckEnabled() && currentEntry?.cookie) {
                    const entry = currentEntry as ApiKeyEntry; // block is disabled; assertion is safe
                    const check = await checkKeyBalance(entry.cookie as string);
                    if (!check.sufficient) {
                        failedKeys.set(entry.value, "balance");
                        await markApiKeyExhausted(this.secrets, entry.value, "balance");
                        logger.warn("key.rotation", {
                            key: maskApiKey(entry.value),
                    ...
                    if (entry.available === false) {
                        await markApiKeyAvailable(this.secrets, entry.value);
                        logger.info("key.recovered", { key: maskApiKey(entry.value) });
                    }
                }
