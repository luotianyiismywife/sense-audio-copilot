...
import type { SenseAudioModelItem, RetryConfig } from "./types";
...
export function normalizeUserModels(models: unknown): SenseAudioModelItem[] {
    const list = Array.isArray(models) ? models : [];
    const out: SenseAudioModelItem[] = [];
    for (const item of list) {
        if (!item || typeof item !== "object") {
            continue;
        }
        const provider = getModelProviderId(item);
        out.push({ ...(item as SenseAudioModelItem), owned_by: provider });
    }
    return out;
}
